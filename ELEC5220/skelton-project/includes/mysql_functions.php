...
{
	$query = "SELECT item_number, brand, name, price, processor, memory, hard_drive, screen_size, image, quantity, weight, description FROM laptop_store WHERE item_number=$item_number";
	...
	if($result)
	{
		...
		$return['hard_drive'] = $row['hard_drive'];
		$return['screen_size'] = $row['screen_size'];
		$return['weight'] = $row['weight'];
		$return['quantity_available'] = $row['quantity'];
...
function get_all_product_infos($start=-1, $length=-1, $type="", $value="")
{
	...
	$query = "SELECT item_number FROM laptop_store $where_clause $limit_clause";
	$result = @mysql_query($query);
	if($result)
	{
...
{
	$query = "SELECT item_number FROM laptop_store ORDER BY RAND() LIMIT $limit";
	$result = @mysql_query($query);
	if($result)
	{
		while($row = @mysql_fetch_assoc($result))
...
{
	$query = "SELECT $type FROM laptop_store ORDER BY $type ASC";
	$result = @mysql_query($query);
	if($result){
		$values = array();
		while($row = @mysql_fetch_assoc($result)){
