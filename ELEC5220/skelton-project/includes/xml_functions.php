...
function update_database($data, $order)
{
	...
	$query1 = "INSERT INTO customers (first_name, last_name, address_1, address_2, city, state, zip, telephone, email, cc_num, cc_type, cc_expire)
	...
	foreach($_SESSION['cart'] as $item_number => $quantity)
	{
		if($quantity > 0)
		{
			$query3 = "UPDATE laptop_store SET quantity=quantity-$quantity WHERE item_number=$item_number";
