...
$dbconn = mysql_connect("localhost", "bob", "bob") or die("Could not connect to database server.");
//select the database "test"
@mysql_select_db("pet_store",$dbconn) or die("Could not connect to pet_store database.");
...
print "<tr><td>Name</td><td>Style</td><td>Cost</td><td>Add New Detail</td></tr>";
while ($row = mysql_fetch_row($res)) {
	...
	print "<td>$row[1]</td>"; //display the title returned
	{
		...
		while($row1 = mysql_fetch_row($res1))
		{
			...
			while($loop < $row[3])
			{
				print "<tr><td><table><tr><td><form action=\"$_SERVER[PHP_SELF]\" method=\"GET\">
	...
	print "<td><table><tr><td><form action=\"$_SERVER[PHP_SELF]\" method=\"GET\">
					<input type=\"hidden\" name=\"ISBN\" value=\"$row[0]\"/>
